...
export default function AddReminderScreen() {
  ...
  const [scheduledDateTime, setScheduledDateTime] = useState('Upcoming Saturday at 10:00 AM');
  const [category, setCategory] = useState<Reminder['category']>('HOSPITAL');
  ...
  const handleDiscard = () => {
    if (title.trim() || voiceNoteUri) {
      Alert.alert('Discard Reminder?', 'Are you sure you want to discard this reminder?', [
  ...
  const handleSave = async () => {
    ...
    try {
      setIsSaving(true);
      await offlineProximitySync.addOneTimeReminder({
        patientId: activePatient.id,
        title: title.trim(),
        description: description.trim(),
        category,
        scheduledTime: scheduledDateTime.trim() || 'Upcoming Saturday at 10:00 AM',
        repeat: 'ONCE',
        alarmEnabled: true,
      ...
      setIsSaving(false);
      Alert.alert('Save Error', e.message || 'Could not save reminder.');
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.topHeader}>
        ...
        <View style={styles.headerTitleWrap}>
          <Text style={styles.headerTitle}>New Reminder / Visit</Text>
      ...
      >
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={styles.presetRow}>
            {[
              ...
              <TouchableOpacity
                key={p}
                onPress={() => setScheduledDateTime(p)}
                style={[styles.presetChip, scheduledDateTime === p && styles.presetChipActive]}
              >
                <Text
                  style={[
                    styles.presetChipText,
                    scheduledDateTime === p && styles.presetChipTextActive,
                  ]}
                >
                  {p}
                </Text>
          ...
          <View style={styles.categoryRow}>
            {(
              [
                ...
                { key: 'APPOINTMENT', label: 'Visit', Icon: Calendar },
              ] as const
            ).map((item) => {
        ...
        {/* Notes */}
        <View style={styles.inputGroup}>
          <Text style={styles.label}>Medical Notes & Instructions (Optional)</Text>
          <TextInput
            style={[styles.textInput, styles.textArea]}
            placeholder="e.g. Carry prescription file, MRI scans, and fasting blood sugar test"
            placeholderTextColor="#94A3B8"
            multiline
            numberOfLines={3}
            value={description}
        ...
        <View style={styles.voiceSectionCard}>
          ...
          <Text style={styles.voiceCardDesc}>
            Record a 5–15s warm voice note. Your loved one will hear your warm, reassuring voice when this reminder triggers instead of an alarming ringtone.
          </Text>
