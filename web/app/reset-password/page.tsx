...
export default function ResetPasswordPage() {
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    if (newPassword !== confirmPassword) {
      setFeedback({ type: 'error', message: t('auth.passwordsMismatch', 'Passwords do not match.') });
      return;
    }
    if (newPassword.length < 6) {
      setFeedback({ type: 'error', message: t('auth.passwordLength', 'Password must be at least 6 characters.') });
      return;
    }
    ...
    try {
      ...
      setFeedback({ type: 'success', message: t('auth.passwordResetSuccess', 'Password updated successfully! Redirecting to Sign In...') });
        ...
        <div style={{ backgroundColor: '#FFFFFF', padding: '40px', borderRadius: '24px', border: '1.5px solid #E2E8F0', width: '100%', maxWidth: '440px', boxShadow: '0 4px 16px rgba(0,0,0,0.03)' }}>
          <div style={{ textAlign: 'center', marginBottom: '28px' }}>
            ...
            <p style={{ margin: 0, fontSize: '14px', color: '#64748B' }}>
              {t('auth.resetPasswordSubtitle', 'Enter a new secure password for your MitraCare account.')}
            </p>
          </div>
          ...
          <form onSubmit={handleReset}>
            ...
            >
              {isLoading ? t('auth.updatingPassword', 'Updating Password...') : t('auth.resetPasswordBtn', 'Reset Password →')}
            </button>
          </form>
        </div>
      </main>
